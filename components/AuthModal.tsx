...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const { signInAnonymous, signInWithGoogle, signInWithEmail, signUpWithEmail } = useAuth();
        ...
        >
          ...
          <div className="p-8">
            ...
            <div className="text-center mb-6">
              <UserCircle2 className="w-16 h-16 mx-auto mb-4 text-cyan-400" />
              <h2 className="text-2xl font-bold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">
                {mode === 'signup' ? 'Create Account' : 'Sign In'}
              </h2>
              <p className="text-sm text-slate-400 mt-2">
                Sign in to save your voice notes
              </p>
            </div>

            {/* Anonymous Sign In (recommended) */}
            {mode === 'anonymous' && (
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="w-full px-6 py-3 rounded-lg border-2 border-slate-300 text-slate-700 font-semibold hover:bg-slate-50 hover:border-slate-400 transition-all"
                >
                  Sign in with Email
                </button>
              </div>
            ...
            {(mode === 'signin' || mode === 'signup') && (
              <form onSubmit={handleEmailAuth} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full px-4 py-2 rounded-lg text-slate-600 hover:bg-slate-100 transition-all text-sm"
                >
                  Continue as Guest
                </button>
              </form>
